...
FastAPI Backend (Python) — orchestrator, no local AI/ML deps
  ├── /ws/chat    — Full pipeline: Audio/Text → STT → Agent → LLM → TTS (cookie/Bearer auth at upgrade)
...
- **HUD overlay (desktop only)**: The Settings toggle creates a second transparent, always-on-top, click-through Tauri window (`hud`, route `/hud`) covering the primary monitor's usable work area (below the macOS menu bar/notch and above the Dock) with an edge ring and four ambient corner widgets. `Cmd+Shift+Space` (global) toggles passive ↔ active (interactive). It is purely presentational: the main window derives `HudStatePayload` from `useAssistant` state and pushes it over Tauri events (`hud://state`, heartbeat every 3 s); the HUD shows "No link to app" after 5 s of silence. There is no backend involvement. See `docs/superpowers/specs/2026-07-03-hud-overlay-design.md`.
...
| WORKER_LOCAL_TIMEOUT | 180.0 | Per-attempt local API timeout (seconds); non-streaming, so the whole generation must fit inside it |
| WORKER_LOCK_KEY | shore:worker:lock | Redis SETNX key for cross-process worker mutex |
| WORKER_LOCK_TTL_SECONDS | 600 | TTL on the Redis lock (must exceed 3 attempts + margin) |
...
| AUTH_COOKIE_SAMESITE | lax | SameSite attribute on the hosted web session cookie |
