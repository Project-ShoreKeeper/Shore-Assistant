...
FastAPI Backend (Python)
  ...
  ├── LLM Agent   (Ollama Qwen2.5-7B via httpx streaming)
  ├── Tools        (system_time, read_file, list_directory, search_web, capture_screen)
  ├── TTS          (edge-tts → ffmpeg → PCM streaming)
  └── Vision       (mss screen capture → Ollama vision model hot-swap)
```
...
│       │   ├── tts_service.py          # edge-tts with ffmpeg PCM decode
...
│       │   └── screen_tools.py         # capture_screen (vision hot-swap)
...
# ffmpeg (required for TTS audio decoding)
...
- **16GB VRAM budget**: Whisper (~1.5GB) + Qwen2.5-7B (~5-6GB). Vision model hot-swaps by unloading LLM first.
...
- **TTS streaming**: LLM tokens accumulate → sentence boundary detected → sentence queued → edge-tts synthesizes → ffmpeg decodes MP3 to PCM → binary frames sent over WebSocket.
...
| VISION_MODEL | llava:7b | Vision model for screen analysis |
