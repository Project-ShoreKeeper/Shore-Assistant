...
GUI-Owl-1.5 serving note: for a llama.cpp/GGUF smoke test, point `EVOCUA_BASE_URL` at a llama-server running a GUI-Owl-1.5-8B-Think GGUF, for example `llama-server -hf mradermacher/GUI-Owl-1.5-8B-Think-GGUF:Q4_K_M --jinja --host 0.0.0.0 --port 8081 --ctx-size 32768`, and set `CUA_MODEL_FORMAT=gui_owl`. The official GUI-Owl card recommends vLLM for safetensors deployments; keep the OpenAI-compatible base URL on port 8081 if using vLLM instead, and pass `--served-model-name gui-owl-1.5-8b-think` — unlike llama-server, vLLM rejects requests whose `model` field (Shore sends the format's `model_label`) does not match a served model name.
| **Terminal** | | |
