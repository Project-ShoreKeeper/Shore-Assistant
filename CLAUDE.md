...
- **Computer use (CUA model)**: Task-driven only; there is no proactive co-pilot pipeline. The `computer_use` tool delegates one concrete GUI task to the configured `CUA_MODEL_FORMAT` (`evocua` by default, or `ui_tars`) on port 8081, then loops screenshot → constrained action parse → native Tauri execution → fresh screenshot. It is admin-only, desktop-only, single-run, capped by `CUA_MAX_STEPS`, and writes one JSONL audit record per action to `CUA_AUDIT_LOG`; screenshots are never persisted. The sub-agent receives only its task, bounded screenshot/action history, and the dedicated GUI system prompt—never Shore memory or profile context. `⌘⇧Esc`, the Chat banner, and the HUD stop action send `cua_abort`.
